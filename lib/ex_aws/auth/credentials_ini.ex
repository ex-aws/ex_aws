if Code.ensure_loaded?(ConfigParser) do
  defmodule ExAws.Auth.CredentialsIni do
    ...
    def parse_ini_file({:ok, contents}, profile_name) do
      ...
      |> case do
        {:ok, %{^profile_name => config}} ->
          ...
          |> strip_key_prefix
        {:ok, %{}} ->
          raise "Missing #{profile_name} entry in configuration"
        _ ->
          %{}
      end
    end
    ...
    def strip_key_prefix(credentials) do
      ...
      |> Enum.filter(fn {key, _} ->
        Enum.member?(~w(aws_access_key_id aws_secret_access_key aws_session_token region), key)
      end)
      |> Enum.map(fn {key, val} ->
        updated_key =
          key
