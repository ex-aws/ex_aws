defmodule ExAws.Config do
  ...
  """
  def get(op, overrides) do
    defaults = ExAws.Config.Defaults.get(op.service)
    common_config = Application.get_all_env(:ex_aws) |> Map.new |> Map.take(@common_config)
    service_config = Application.get_env(:ex_aws, op.service) |> Map.new
